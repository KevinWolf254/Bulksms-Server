...
@Repository
public class MonthScheduleRepositoryImpl implements MonthScheduleRepository{
	...
	public ResponseEntity<PagedMonthSchedule> findAll(int schedulesPerViewPage, int firstResult, int maxResults){
		...
		PagedMonthSchedule response = null;
		int totalPages = 0;
		try {
			...
			List<MonthSchedule> scheduledData = criteria.list();
			...
			response = new PagedMonthSchedule(scheduledData, pageResponse);			
		...
		return new ResponseEntity<PagedMonthSchedule>(response, HttpStatus.OK);
	}
	
	@Override
	public MonthSchedule findById(Long scheduleId) {
		Session session = sessionFactory.openSession();
		MonthSchedule schedule = null;
		try {
			schedule = session.get(MonthSchedule.class, scheduleId);
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
			session.close();
	...
	private MonthSchedule findById(Long scheduleId, Session session) {
		MonthSchedule schedule = null;
		Criteria criteria = session.createCriteria(MonthSchedule.class);
		criteria.add(Restrictions.eqOrIsNull("scheduleId", scheduleId));			
		schedule = (MonthSchedule) criteria.uniqueResult();
		return schedule;	
	}
	...
	public ResponseEntity<Object> findByDate(String date) {
		Session session = sessionFactory.openSession();
		List<MonthSchedule> schedules = null;
		try {
			Criteria criteria = session.createCriteria(MonthSchedule.class);
	...
	public ResponseEntity<Object> findByGroup(Long groupId) {
		...
		List<MonthSchedule> schedules = null;
		try {
			Criteria scheduleCriteria = session.createCriteria(MonthSchedule.class);
	...
	private void save(MonthSchedule schedule) {
		Session session = sessionFactory.openSession();
	...
	public ResponseEntity<Object> updateSchedule(MonthSchedule schedule) {
		...
		MonthSchedule search = null;
		
		MonthSchedule updatedSchedule = null;
		responseList = new ArrayList<>();
		response = new HashMap<>();
		try {
			Criteria criteria = session.createCriteria(MonthSchedule.class);
			criteria.add(Restrictions.eqOrIsNull("title", schedule.getTitle()));
			search = (MonthSchedule) criteria.uniqueResult();
			 if(search == null) {
				updatedSchedule = (MonthSchedule) session.merge(schedule);
				trans.commit();
	...
	public ResponseEntity<Object> deleteSchedule(Long scheduleId) {
		...
		response = new HashMap<>();
		try {
			MonthSchedule schedule = findById(scheduleId, session);
			if(schedule == null) {
			...
			schedule.getMonthlyGroups().removeAll(schedule.getMonthlyGroups());
