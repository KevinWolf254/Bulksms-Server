...
import co.ke.bigfootke.app.services.WeekScheduleService;

@RestController
@RequestMapping(value = "api/schedule/week")
public class WeekScheduleController {
	...
	public ResponseEntity<Object> createSchedule(@RequestBody @Valid WeekSchedule schedule,  BindingResult bindingResult) {
	...
	public ResponseEntity<PagedWeekSchedule> findAll(@PathVariable int firstResult) {
	...
	public ResponseEntity<Object> findByTitle(@PathVariable String title) {
		WeekSchedule schedule = service.findByTitle(title);
		if(schedule == null) {
			response = new HashMap<>();
	...
	public ResponseEntity<Object> updateSchedule(@RequestBody WeekSchedule schedule) {
